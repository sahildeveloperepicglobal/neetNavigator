...
import { useUpdateStory } from "@/network-requests/mutations";
import { useGetStoryById } from "@/network-requests/queries";
...
const UpdateStory = () => {
  ...
  const { data, isFetched, isLoading } = useGetStoryById(
    router.query.id as string
  );
  const { mutate } = useUpdateStory(router.query.id as string);
  ...
  const onSubmitData = React.useCallback(async () => {
    ...
    mutate(data, {
      onSuccess: (data: any) => {
        router.back();
        alert(data?.message || "Story Updated Successfully");
      },
      onError: () => {
